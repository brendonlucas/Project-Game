...
func _process(delta):
	...
	if !player and !timer_iniciado:
		...
		Gamestate.drop_game()
	if enemys_kill >= 8 and door_close:
...
func up_speed_ataque():
	enemys_kill += 1
	if get_node_or_null("enemy_hack"):
		$enemy_hack.up_bullet_speed()
	if get_node_or_null("enemy_hack2"):
		$enemy_hack2.up_bullet_speed()
	if get_node_or_null("enemy_hack3"):
		$enemy_hack3.up_bullet_speed()
	if get_node_or_null("enemy_hack4"):
		$enemy_hack4.up_bullet_speed()
	if get_node_or_null("enemy_hack5"):
		$enemy_hack5.up_bullet_speed()
	if get_node_or_null("enemy_hack6"):
		$enemy_hack6.up_bullet_speed()
